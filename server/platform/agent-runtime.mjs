...
import {
  loadSession,
  saveSession,
} from "../persistence/session-repository.mjs";
...
async function buildAgent({
  ...
  return new HarnessAgent({
    ...
    instructions:
      phase === "impl"
        ? instructions + environmentHint(toolCfg.tools)
        : instructions,
    ...toolSettings,
...
async function grabJsonl(sessionId) {
  ...
  try {
    ...
    return {
      jsonl: await readFile(path.join(dir, newest.f), "utf8"),
      sessionFileName: newest.f,
    };
  } catch {
    return null;
  }
...
export async function runTurn({
  ...
  try {
    ...
    await saveSession(sessionId, {
      ...
      sessionFileName:
        grabbed?.sessionFileName ??
        resumeState?.data?.sessionFileName ??
        saved?.sessionFileName ??
        null,
    });
  }
  return out;

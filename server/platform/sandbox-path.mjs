 ...
 *   - Windows 上压根没有这个二进制；
 *   - 就算有，桥接过去的参数是沙箱路径（/workspace/src/a.js），宿主上并不存在。
 * 只能在沙箱内自己实现一个，直接走沙箱 VFS。
 *
 * 【二】Windows 上 ReadWriteFs 交出来的"虚拟路径"带反斜杠
 *
 * 它的做法是把宿主真实路径的 root 前缀切掉：
 *     realpath("/workspace/src/a.js")
 *       → 宿主 "D:\trees\workspace\src\a.js"，slice 掉 "D:\trees"
 *       → "\workspace\src\a.js"          ← 反斜杠，不是合法的 posix 绝对路径
 *
...
const absolutize = (cwd, p) =>
  normalizeAbs(p.startsWith("/") ? p : `${cwd || "/"}/${p}`);
    ...
    for (const t of targets) {
      try {
        ...
        if (symlinks) {
          ...
          if (mustExist && !(await ctx.fs.exists(abs)))

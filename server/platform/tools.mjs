...
export function agentToolProfile(phase, projectDir) {
  ...
  try {
    const cfg = JSON.parse(
      readFileSync(path.join(projectDir, CONFIG_NAME), "utf8"),
    );
...
export function agentToolSettings(phase, projectDir) {
  ...
  return {
    ...
    ...(p.allow
      ? { builtinToolFiltering: { mode: "allow", toolNames: p.allow } }
      : {}),
  };
}
...
export function resolveToolConfig(projectDir) {
  const fromEnv = {
    tools: (process.env.SANDBOX_TOOLS ?? DEFAULT_TOOLS)
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean),
    ...
    return {
      tools:
        Array.isArray(list) && list.length
          ...
          : fromEnv.tools,
      timeout:
        Number(cfg.toolTimeoutMs) > 0
          ? Number(cfg.toolTimeoutMs)
          : fromEnv.timeout,
      source: CONFIG_NAME,
    };
  } catch {
...
function bridge(name, root, timeout) {
  return defineCommand(name, async (args, ctx) => {
    // 沙箱 cwd 形如 /workspace/sub → 宿主 <root>/workspace/sub
...
export function registerTools(sandbox, root, projectDir) {
  ...
  for (const n of tools)
    sandbox.bashEnvInstance.registerCommand(bridge(n, root, timeout));

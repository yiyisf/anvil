...
import { BUILD_ACTIVITY_DEFINITIONS, createBuildActivities } from "./build-flow.mjs";
...
test("BUILD only gates alignment and planning", () => {
  ...
  assert.equal(activities.find((x) => x.type === "alignment").gate.kind, "approval");
  assert.equal(activities.find((x) => x.type === "specification").gate, null);
  assert.equal(activities.find((x) => x.type === "planning").gate.kind, "approval");
});
...
test("parses local Matt ticket and blockers", () => {
  ...
  assert.equal(ticket.id, "02"); assert.deepEqual(ticket.dependencies, ["01"]);
});
...
test("discovers Matt .scratch feature issue files", async (t) => {
  ...
  assert.equal(found.featureSlug, "member-login"); assert.deepEqual(found.tickets.map((x) => x.id), ["01", "02"]); assert.deepEqual(ticketFrontier(found.tickets).map((x) => x.id), ["01"]);
});
...
test("legacy req can be projected without destructive migration", () => {
  ...
  assert.equal(work.mode, "build"); assert.equal(work.compatibility, true);
});
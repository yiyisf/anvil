...
import { newActivity, newAgentSession, newSkillRun } from "./domain-model.mjs";
...
function escalationGate(decision, interruption) {
  ...
  return {
    ...
    options: exhausted
      ? [{ id: "retry", label: "再试一次", action: "retry" }, { id: "replan", label: "重新规划", action: "replan" }, { id: "stop", label: "停止 BUILD", action: "stop" }]
      : [{ id: "retry", label: "条件已处理，继续", action: "retry" }, { id: "stop", label: "停止 BUILD", action: "stop" }],
...
export async function planRecovery({ error, work, activity, runId = null }) {
  ...
  const history = await listInterruptions(work.id, activity.id); const decision = recoveryDecision(interruption, history);
  ...
  interruption.status = decision.action === "recover" ? "recovering" : "waiting_user"; await saveInterruption(interruption);
  ...
  await saveActivity(activity); await saveWork(work); return { interruption, decision };
}
...
async function runRecoverySkill({ work, activity, interruption, project, onEvent }) {
  ...
  const skills = await loadMattSkillBundle(skill); const prompt = buildSkillInvocation(skill, `处理 Activity ${activity.id} 的恢复问题。证据：${interruption.evidence?.message || "未知"}`);
  try {
...
export async function executeRecovery({ work, activity, interruption, project, runOriginal, onEvent }) {
  const strategy = interruption.recovery?.strategy; if (!strategy || strategy === "human_gate") return { recovered: false, reason: "human_gate" };
  try {
    if (strategy === "skill_recovery") await runRecoverySkill({ work, activity, interruption, project, onEvent });
    const fresh = strategy === "fresh_session" || strategy === "skill_recovery"; let prompt = retryPrompt(interruption);
    if (fresh) {
      try { const handoff = await createSessionHandoff({ work, activity, project, onEvent }); activity.handoff = handoff; await saveActivity(activity); prompt = continuationPrompt(handoff, prompt); }
      catch (handoffError) { activity.handoff = { text: "", error: handoffError.message, createdAt: new Date().toISOString() }; await saveActivity(activity); prompt = continuationPrompt(activity.handoff, prompt); }
    }
    const result = await runOriginal({ prompt, freshSession: fresh }); resolveInterruption(interruption); await saveInterruption(interruption); return { recovered: true, result };
  } catch (error) {
    interruption.status = "failed_recovery"; interruption.updatedAt = new Date().toISOString(); await saveInterruption(interruption); return { recovered: false, error };
  }
}
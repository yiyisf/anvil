...
const routes = {
  ...
  "POST /api/impl-message": async ({ q, body }) => { const req = await getReq(q.id); if (!req) return { error: "not found" }; const project = await requireProject(req); req.implChat.push({ role: "user", text: body.text }); const cur = req.tickets?.find((t) => t.skills.some((s) => s.state === "running")) || req.tickets?.find((t) => t.skills.some((s) => s.state === "fail" || s.state === "waiting")) || req.tickets?.slice().reverse().find((t) => t.skills.some((s) => s.state !== "idle")) || req.tickets?.[0]; const out = await runTurn({ reqId: req.id, worktreesDir: project.worktreesDir, sessionKey: cur ? `impl-${cur.ticket}` : "impl", phase: "impl", instructions: IMPL_INSTRUCTIONS, skills: SKILLS, prompt: body.text }); req.implChat.push({ role: "assistant", text: extractVerdict(out.text).clean }); await saveReq(req); return req; },
...
function handleEvents(rq, rs, reqId) { rs.writeHead(200, { "content-type": "application/x-ndjson; charset=utf-8", "cache-control": "no-cache", "x-accel-buffering": "no", connection: "keep-alive" }); const write = (e) => { if (!rs.writableEnded) rs.write(JSON.stringify(e) + "\n"); }; if (!isRunning(reqId)) { write({ type: "run", state: "idle" }); return rs.end(); } const unsub = subscribe(reqId, (e) => { write(e); if (e.type === "closed") { unsub(); rs.end(); } }); const hb = setInterval(() => write({ type: "ping" }), 20000); rq.on("close", () => { clearInterval(hb); unsub(); }); rs.on("close", () => { clearInterval(hb); unsub(); }); }
function matchParamRoute(table, method, pathname) { for (const key of Object.keys(table)) { const [m, pat] = key.split(" "); if (m !== method || !pat.includes(":")) continue; const re = new RegExp("^" + pat.replace(/:[^/]+/g, "([^/]+)") + "$"); const mm = pathname.match(re); if (mm) return { handler: table[key], params: mm.slice(1) }; } return null; }
const server = http.createServer(async (rq, rs) => { const url = new URL(rq.url, "http://x"); const key = `${rq.method} ${url.pathname}`; if (key === "GET /api/events") { rs.setHeader("Access-Control-Allow-Origin", "*"); return handleEvents(rq, rs, url.searchParams.get("id")); } let handler = routes[key], streamHandler = streamRoutes[key], paramId = null; if (!handler && !streamHandler) { const m = matchParamRoute(routes, rq.method, url.pathname); if (m) { handler = m.handler; paramId = m.params[0]; } if (!handler) { const sm = matchParamRoute(streamRoutes, rq.method, url.pathname); if (sm) { streamHandler = sm.handler; paramId = sm.params[0]; } } } rs.setHeader("Access-Control-Allow-Origin", "*"); rs.setHeader("Access-Control-Allow-Headers", "content-type"); if (rq.method === "OPTIONS") return rs.writeHead(204).end(); if (!handler && !streamHandler) return rs.writeHead(404).end("not found"); try { let body = {}; if (rq.method === "POST") { const chunks = []; for await (const c of rq) chunks.push(c); const raw = Buffer.concat(chunks).toString("utf8"); body = raw ? JSON.parse(raw) : {}; } const q = Object.fromEntries(url.searchParams); if (paramId) q.id = paramId; if (streamHandler) { rs.writeHead(200, { "content-type": "application/x-ndjson; charset=utf-8", "cache-control": "no-cache", "x-accel-buffering": "no" }); const emit = (e) => { if (!rs.writableEnded) rs.write(JSON.stringify(e) + "\n"); }; try { const req = await streamHandler({ body, q, emit }); emit({ type: "done", req }); } catch (e) { console.error(`[${key}]`, e); emit({ type: "error", message: e.message }); } return rs.end(); } const data = await handler({ body, q }); rs.writeHead(200, { "content-type": "application/json; charset=utf-8" }); rs.end(JSON.stringify(data)); } catch (e) { console.error(`[${key}]`, e); rs.writeHead(500, { "content-type": "application/json; charset=utf-8" }); rs.end(JSON.stringify({ error: e.message })); } });
migrateLegacyProject().catch((e) => console.error("[migrate]", e)).finally(() => { server.listen(PORT, () => { console.log(`API  http://localhost:${PORT}`); console.log(`模型 ${process.env.PI_MODEL} via ${process.env.LITELLM_BASE_URL}`); const c = resolveToolConfig(); console.log(`默认 sandbox 工具 [${c.tools.join(",")}]（来源 ${c.source}；项目 agent.config.json 会覆盖）`); console.log(`排查工具链问题：node --env-file=.env doctor.mjs`); }); });

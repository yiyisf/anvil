...
export const WORK_STATUSES = new Set(["draft", "active", "waiting_user", "completed", "failed", "cancelled"]);
...
export function newWork({ projectId, title, mode = "build", legacyReqId = null }) {
  ...
  const timestamp = now();
  return { id: id("WORK"), projectId, legacyReqId, title: title?.trim() || "未命名工作", mode, status: "draft", currentActivityId: null, workspace: {}, buildRoute: null, createdAt: timestamp, updatedAt: timestamp };
}

export function newActivity({ workId, type, label, skill = null, ticketId = null, gate = null, parentActivityId = null, recoveryFor = null }) {
  if (!workId) throw new Error("workId 必填");
  const timestamp = now();
  return {
    id: id("ACT"), workId, ticketId, type, label, status: "idle",
    parentActivityId, recoveryFor,
    technical: skill ? { skill } : {},
    gate: gate ? { kind: gate.kind || "approval", required: gate.required !== false, status: "pending", prompt: gate.prompt || null, decision: null, decidedAt: null } : null,

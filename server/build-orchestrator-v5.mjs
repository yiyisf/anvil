...
export async function decideHumanGate({ workId, activityId, decision, route = null }) {
  ...
  if (activity.type === "alignment" && decision === "approve") setBuildRoute(work, route || activity.routeRecommendation?.route || "tickets");
...
async function finishSingleSessionImplementation({ work, activity, project, prompt, onEvent }) {
  await continueActivitySession({ workId: work.id, activityId: activity.id, project, phase: "impl", prompt, onEvent });
  work.status = "completed"; work.currentActivityId = null; await saveWork(work); return { reason: "completed", work };
}

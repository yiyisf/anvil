...
export async function createSessionHandoff({
  ...
  const out = await runTurn({
    ...
    instructions:
      "Prepare a concise handoff for a fresh agent session. Preserve decisions, current state, blockers, next action, and references to durable artifacts. Do not continue implementation.",
    skills,
    prompt: buildSkillInvocation(
      "handoff",
      "Prepare a continuation handoff for the current Anvil activity.",
    ),
    onEvent,
  });
  return {
...
export function continuationPrompt(handoff, originalPrompt = "") {
  ...
  return [
    ...
    "",
    handoff?.text ||
      "No handoff text was available; recover state from durable worktree artifacts.",

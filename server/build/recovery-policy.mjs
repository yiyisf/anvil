...
export function recoveryDecision(
  ...
  const automatic = history.filter(
    (x) => x.recovery?.strategy && x.recovery.strategy !== "human_gate",
  ).length;
  const same = history.filter(
    (x) => x.signature === interruption.signature,
  ).length;
  if (interruption.severity === "blocked" || spec.strategy === "human_gate")
    return { ...spec, action: "human_gate" };
  if (
    automatic >= policy.maxAutomaticRecoveries ||
    same >= Math.min(spec.maxAttempts, policy.maxSameFailure)
  ) {
    return {
      strategy: "human_gate",
      gateKind: "escalation",

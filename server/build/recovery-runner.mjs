...
import { newActivity, newAgentSession, newSkillRun } from "./domain.mjs";
...
function escalationGate(decision, interruption) {
  ...
  return {
    kind: decision.gateKind || "escalation",
    required: true,
    status: "waiting",
    prompt: exhausted
      ? "自动恢复已达到安全上限，需要决定如何继续。"
      : "执行需要外部条件或人工判断后才能继续。",
    context: {
...
export async function planRecovery({ error, work, activity, runId = null }) {
  ...
  interruption.status =
    decision.action === "recover" ? "recovering" : "waiting_user";
  ...
  if (decision.action === "recover") {
    ...
    activity.gate = escalationGate(decision, interruption);
...
async function runRecoverySkill({
  work,
  activity,
  interruption,
  project,
  ...
  const recovery = newActivity({
    ...
    recoveryFor: interruption.id,
  });
  const session = newAgentSession({
    workId: work.id,
    activityId: recovery.id,
    ticketId: activity.ticketId,
    continuationOf: activity.sessionId,
  });
  ...
  recovery.startedAt = new Date().toISOString();
  session.status = "active";
  run.status = "running";
  run.startedAt = recovery.startedAt;
  await saveActivity(recovery);
  await saveAgentSession(session);
  await saveSkillRun(run);
  const skills = await loadMattSkillBundle(skill);
  const prompt = buildSkillInvocation(
    skill,
    `处理 Activity ${activity.id} 的恢复问题。证据：${interruption.evidence?.message || "未知"}`,
  );
  try {
    const out = await runTurn({
      reqId: work.id,
      worktreesDir: project.worktreesDir,
      sessionKey: `recovery-${recovery.id}`,
      phase: "impl",
      instructions:
        "You are executing a bounded recovery activity. Resolve only the interruption, verify the repository is safe, then stop.",
      skills,
      prompt,
      onEvent,
    });
...
export async function executeRecovery({
  ...
  if (!strategy || strategy === "human_gate")
    return { recovered: false, reason: "human_gate" };
  try {
    if (strategy === "skill_recovery")
      await runRecoverySkill({
        work,
        activity,
        interruption,
        project,
    ...
    interruption.status = "failed_recovery";
    interruption.updatedAt = new Date().toISOString();
    await saveInterruption(interruption);

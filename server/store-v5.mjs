...
async function save(kind, value) {
  ...
  await fs.writeFile(file(dir, value.id), JSON.stringify(value, null, 2));
  return value;
}

async function get(kind, id) {
  try { return JSON.parse(await fs.readFile(file(dirs[kind], id), "utf8")); }
  catch (error) { if (error.code === "ENOENT") return null; throw error; }
}

async function list(kind, predicate = () => true) {
  const dir = dirs[kind];
  await ensure(dir);
  const names = await fs.readdir(dir);
  const values = [];
  for (const name of names.filter((x) => x.endsWith(".json"))) {
    const value = JSON.parse(await fs.readFile(path.join(dir, name), "utf8"));

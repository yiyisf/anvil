import { createImplementationActivity } from "./build-flow.mjs";
import { getTicketFrontier } from "./ticket-frontier-v5.mjs";
import { listActivities, saveActivity, getWork } from "./store-v5.mjs";
...
export async function ensureImplementationActivities({ work, project }) {
  ...
  const byTicket = new Map(
    activities.filter((a) => a.type === "implementation" && a.ticketId).map((a) => [a.ticketId, a]),
  );
  const created = [];
...
export async function runFrontierTicket({ workId, ticketId = null, project, onEvent }) {
  ...
  if (!ticket) throw new Error(ticketId ? `Ticket ${ticketId} 当前不在 frontier` : "当前没有可执行的 Ticket");
  const activity = prepared.activities.find((a) => a.type === "implementation" && a.ticketId === ticket.id);
  ...
  return runActivity({ workId, activityId: activity.id, project, prompt, onEvent });
}

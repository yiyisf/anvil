...
export async function runActivity({ workId, activityId, project, prompt = "", onEvent }) {
  ...
  const invocation = buildSkillInvocation(skill, prompt); const skills = await loadMattSkillBundle(skill);
  try {
    const out = await runTurn({ reqId: work.id, worktreesDir: project.worktreesDir, sessionKey: `activity-${activity.id}`, phase: ACTIVITY_PHASE[activity.type] || "impl", instructions: instructionsFor(activity), skills, prompt: invocation, onEvent });
    ...
    run.status = "failed"; run.summary = error.message; run.finishedAt = new Date().toISOString(); session.status = "failed"; activity.status = "failed"; activity.finishedAt = new Date().toISOString(); work.status = "failed";
    await saveSkillRun(run); await saveAgentSession(session); await saveActivity(activity); await saveWork(work); throw error;
  }
}

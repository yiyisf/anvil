...
- **直接开发**：小而明确的改动沿用当前 Coding Agent Session 的上下文，但进入独立的「开发实现」Activity，不强制生成 Spec 或 Tickets。
...
1. 创建一个 BUILD，只需要用业务语言描述希望完成的事情；创建成功后左侧项目栏自动收起，把主界面留给对话。
2. AI 自动检查项目并进入需求澄清；存在关键边界时通过对话继续确认。回答会立即显示，对话随 AI 输出自动滚动。
3. 右侧「需求理解」只保留模型生成的简短问题摘要与实际决策结论，不复制对话全文或思考过程；同时展示需求概述、仍待确认内容、执行路线和可能影响区域。
4. 当需求足够明确时，Agent 结构化选择直接开发、先形成方案或拆分开发任务。
5. 低风险需求会先在页面显示自动选择的执行方式和原因，再继续推进，不要求重复点击确认；高风险、不可逆或涉及外部系统的操作才生成动态确认。
...
- direct：需求明确后自动进入同 Session 实现；
- spec：自动生成规格后直接实现，不进入 Ticket 拆分；
- tickets：仅在明确选择拆分时自动生成任务并按 Ticket Frontier 实现；
- decision：缺失或非法路线不会静默回退到 tickets，高风险路线才产生动态 Gate。

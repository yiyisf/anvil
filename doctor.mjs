 ...
 *
 * 运行：node doctor.mjs [需求ID]
 *   不传需求ID时，检查 WORKTREES_DIR 下第一个目录。
 */
...
if (!WORKTREES_DIR) { console.log("未设置 WORKTREES_DIR，请用 node --env-file=.env doctor.mjs"); process.exit(1); }

/* ── 0. 环境（doctor 与服务必须看同一个目录）── */
line("【0】环境");
line(`  当前工作目录：${process.cwd()}`);
line(`  WORKTREES_DIR：${WORKTREES_DIR}${WORKTREES_DIR !== abs(WORKTREES_DIR) ? `  → 解析为 ${abs(WORKTREES_DIR)}` : ""}`);
line(`  REPO_PATH    ：${REPO_PATH}${REPO_PATH && REPO_PATH !== abs(REPO_PATH) ? `  → 解析为 ${abs(REPO_PATH)}` : ""}`);
// Windows 上用 POSIX 风格路径（/d/trees、/c/repo）Node 解析不了
if (process.platform === "win32" && /^\/[a-zA-Z]\//.test(String(WORKTREES_DIR || "") + String(REPO_PATH || ""))) {
  line("");
...
if (WORKTREES_DIR !== abs(WORKTREES_DIR)) {
  ...
  line("    解析出的绝对路径就不同，会出现「服务建好了但 doctor 找不到」。建议改成绝对路径。");
}
...
try {
  const out = execFileSync("git", ["worktree", "list"], { cwd: REPO_PATH, encoding: "utf8" }).trim();
  for (const l of out.split("\n")) line("    " + l);
} catch (e) {
  no("  读不到（REPO_PATH 不对，或不是 git 仓库）：" + String(e.message).split("\n")[0]);
}

line("");
line("  WORKTREES_DIR 下的条目：");
try {
  const entries = readdirSync(abs(WORKTREES_DIR)).filter((d) => !d.startsWith("."));
  if (!entries.length) line("    （空）");
  for (const d of entries) {
    const full = path.join(abs(WORKTREES_DIR), d);
    line(`    ${d}  ${existsSync(path.join(full, ".git")) ? "✔ 是 worktree" : "✗ 不是 worktree（缺 .git）"}`);
  }
} catch {
...
let reqId = process.argv[2];
if (!reqId) {
  const dirs = existsSync(ROOT)
    ? readdirSync(ROOT).filter((d) => !d.startsWith(".") && isWorktree(path.join(ROOT, d)))
    : [];
  reqId = dirs[0];
}
if (!reqId) {
  ...
  line("");
  line("  先在界面里创建一个需求（会自动建 worktree），再跑 doctor。");
  line("  也可以指定：node --env-file=.env doctor.mjs REQ-XXXXX");
  line("");
...
const tree = path.join(ROOT, reqId);
if (!existsSync(tree)) { no(`${tree} 不存在`); process.exit(1); }
if (!isWorktree(tree)) {
...
if (hasTree) ok(`worktree 里有：${inTree}`);
  ...
  if (hasRepo) {
    ...
    line("    worktree 是从分支检出的，【未提交的文件不会出现在 worktree 里】。");
    line("    解决：在主仓库把它提交进去");
    line(`      cd ${REPO_PATH} && git add agent.config.json && git commit -m "add agent config"`);
...
if (hasTree) {
  ...
  } catch (e) { no("解析失败，格式有误：" + e.message); }

  // worktree 是创建那一刻从分支检出的：之后往主仓库提交的新配置不会同步过来
  if (hasRepo && treeCfg) {
    let repoCfg = null;
    try { repoCfg = JSON.parse(readFileSync(inRepo, "utf8")); } catch {}
...
if (ap.allow && !ap.allow.includes("bash")) {
  no("impl 阶段没有 bash 工具 → 代理根本无法执行命令，sandboxTools 配了也用不上");
}
...
for (const t of cfg.tools) {
  ...
  if (!bin) { no(`${t} 不在 PATH 中 —— 桥接会返回「宿主上找不到 ${t}」`); continue; }
  ok(`${t} → ${bin.path}${bin.isBatch ? "   [批处理，将经 cmd.exe 启动]" : ""}`);
}
if (process.platform === "win32") {
  line("  （Windows 提示：mvn/npm/npx/gradle 通常是 .cmd 批处理，");
  line("    必须经 cmd.exe 启动；桥接已按解析出的扩展名自动处理）");
}
line(`  （服务进程的 PATH：${(process.env.PATH || "").split(path.delimiter).slice(0, 6).join(path.delimiter)} …）`);
...
for (const t of registered.tools) {
  // 不用管道，避免 head 把退出码吃掉
  const r = await session.run({ command: `cd /${reqId} && ${t} --version 2>&1` });
  const out = (r.stdout || r.stderr || "").trim().split("\n")[0] || "(无输出)";
  if (/command not found/.test(out)) no(`${t}：未注册进 bash —— 配置没读到，或服务未重启`);
  else if (/工作目录不存在/.test(out)) no(`${t}：工作目录不存在 —— worktree 缺失，与命令本身无关`);
  else if (/宿主上找不到命令/.test(out)) no(`${t}：已注册，但服务进程的 PATH 里没有它`);
